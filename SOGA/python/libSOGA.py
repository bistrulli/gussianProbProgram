...
def start_SOGA(cfg, pruning=None, Kmax=None):
    ...
    SOGA(cfg.root, 1, init_dist, None, data)
    start = time()
...
def SOGA(node, current_p, current_dist, current_trunc, data):
        
    # starts execution
    if node.type == 'entry':
        for child in node.children:
            SOGA(child, deepcopy(current_p), deepcopy(current_dist), deepcopy(current_trunc), data)
    ...
    if node.type == 'state':
        if node.cond != None and not current_trunc is None:
            ...
            start = time()
            p, current_dist = truncate(current_dist, current_trunc, data)   ### see libSOGAtruncate
            current_trunc = None
            end = time()
        ...
        SOGA(node.children[0], deepcopy(current_p), deepcopy(current_dist), deepcopy(current_trunc), data)
            
    # if observe truncates to LBC and calls on children
    if node.type == 'observe':
        current_trunc = node.LBC
        start = time()
        _, current_dist = truncate(current_dist, current_trunc, data)
        current_trunc = None
        end = time()
        timing.trunc_time.append(end-start)
        for child in node.children:
